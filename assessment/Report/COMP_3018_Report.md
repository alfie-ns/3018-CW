...
- [ ] Furthermore, GAZE implements selectable personality modes grounded in the adaptive personality-matching paradigm identified by Tapus, Tapus and Mataric (2008, p. TODO: VERIFY PAGE), who demonstrate that adjusting a rehabilitation robot's interaction style (interaction distance and speed) to the individual user's personality traits improved task performance. Kahn et al. (2008, pp. 97-104) formalise this within their design patterns for sociality in human-robot interaction, proposing that robots should exhibit distinct social personas rather than a single generic affect. GAZE operationalises this via four user-selectable personality modes (Cheeky and Playful, Warm and Wise Mentor, Enthusiastic Coach, Calm and Therapeutic), each injecting a distinct behavioural fragment into the LLM's system prompt. The personality selection is persisted across sessions and the system can suggest switching personality mid-session if emotional signals indicate a mismatch between the active mode and the user's inferred state; this adaptive switching extends Tapus et al.'s static matching to a dynamic, signal-driven approach.

## 2.3. Methods & Setup (35%; Alfie)

### 2.3.1 System Architecture

GAZE operates as a conversational loop rather than a rigid question-answer cycle. Each turn, the system: 1) captures all five emotional signals from the user (facial expression, vocal emotion, speech volume, response time, and, during active games, answer correctness); 2) fuses these via the AdaptiveEngine to infer the user's emotional-cognitive state; 3) packages the signal context alongside the user's transcribed speech and sends both to OpenAI GPT-4.1 with function-calling tools enabled; 4) the LLM decides autonomously whether to respond conversationally, initiate a game question, check a game answer, adjust difficulty, or perform any other action by invoking the appropriate tool; 5) the final spoken response, alongside context-aligned gestures and LED state, is delivered on Pepper concurrently via threading. Computationally intensive processing (CNN inference, OpenAI API calls, adaptive-engine logic) runs on the laptop, whilst Pepper handles only physical I/O.

- [ ] This function-calling architecture represents a neuro-symbolic design wherein the neural subsystem (GPT-4.1) governs natural dialogue and high-level decision-making, whilst symbolic subsystems (the AdaptiveEngine, game logic, scoring) are exposed as callable tools that the LLM invokes when appropriate. This aligns with the 'third wave' paradigm identified by Garcez and Lamb (2023, p. TODO: VERIFY 12389), wherein neural and symbolic components share a structured interface rather than operating in isolation. The LLM is thereby grounded in Pepper's actual capabilities via tool definitions, extending the affordance-aware principle advocated by Ahn et al. (2022, p. 1) from action selection to conversational autonomy.
...
- [ ] Rather than constructing a fixed game prompt each round, GAZE utilises OpenAI function calling to let GPT-4.1 autonomously decide which actions to take. Every turn, `build_signal_context()` packages all five live signals (facial expression, vocal emotion, speech volume, response time, rolling correctness) into a concise context block that is prepended to the user's transcribed speech. The conversation, including this signal context, is sent to GPT-4.1 with eight callable tools: `generate_game_question` (generate a countdown-style numbers or letters question at a specified difficulty), `check_game_answer` (verify the user's spoken answer against the correct answer), `get_adaptive_recommendation` (query the AdaptiveEngine for its signal-based recommendation), `check_reward_milestone` (check for scoring achievements), `get_session_summary` (retrieve session statistics), `save_progress` (persist session state), `evaluate_last_adaptation` (self-evaluate whether the previous adaptation worked), and `select_personality` (switch the robot's personality mode mid-session). The LLM decides which tools to invoke based on conversational context; during natural conversation it calls none, whilst during active gameplay it chains `check_game_answer` followed by `get_adaptive_recommendation` and `generate_game_question` in a single turn. Separate OpenAI calls handle game-question generation at a creative temperature and answer verification at a deterministic temperature; the latter mitigates the hallucination risk Ji et al. (2023, p. 3) identify in LLM-generated output. A strict 10-second timeout wraps every API call; if the network stalls, the robot falls back gracefully and proceeds, thereby ensuring Pepper remains responsive.
...
- [ ] The conversational architecture, wherein the LLM autonomously decides when to play games, chat, encourage, or check in on the user via function calling, positions GAZE as a social companion rather than a rigid game host. The selectable personality modes (Cheeky and Playful, Warm and Wise Mentor, Enthusiastic Coach, Calm and Therapeutic) extend the adaptive personality-matching paradigm identified by Tapus, Tapus and Mataric (2008, p. TODO: VERIFY PAGE) from static trait-matching to dynamic, signal-driven adaptation wherein the system can suggest switching personality mid-session if emotional signals indicate a mismatch.

The multi-signal approach could transfer beyond game-based engagement to stroke rehabilitation re-engagement (Mataric et al., 2007), educational tutoring, or therapeutic support for neurodivergent users. Future work could replace the hand-coded classification thresholds with learned parameters derived from longitudinal user data, and fine-tune both models on in-session Pepper captures to improve classification accuracy within the specific deployment environment.
