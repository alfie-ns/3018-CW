...
def transcribe(bypass_wake_word: bool = False,
               ...
               record_again=None,
               max_hallucination_retries: int = 1) -> str:
    """
    ...
    and Whisper repetition-loop hallucinations trigger a fresh listen rather
    than a silent drop, up to max_hallucination_retries extra attempts.
    """
    ...
    while True:
        ...
        try:
            ...
            # normalised hallucination blacklist + repetition loop
            if repetition_loop or is_known_hallucination(text):
                print(f"  Filtered Whisper hallucination: {text!r}")
                if record_again is not None and attempts_remaining > 0:
...
{{"question": "...", "answer": "...", "category": "..."}}"""
    ...
    try:
        resp = client.chat.completions.create(
            model="gpt-4.1",
            messages=[
                {"role": "system", "content": "You generate countdown-style game questions. Respond ONLY with valid JSON. Follow all rules in the user prompt exactly. For numbers rounds you MUST verify your own arithmetic before responding — work out the solution step by step and confirm the target is actually reachable from the given numbers before writing the answer field."},
