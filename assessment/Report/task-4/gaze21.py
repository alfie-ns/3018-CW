...
Modes:
    LOCAL_MODE=true   webcam, Mac/Windows mic, local TTS; Pepper not required.
    LOCAL_MODE=false  Pepper over SSH; output (TTS, LEDs, gestures) on robot.
    HYBRID_LOCAL_INPUT (gaze21 default = True): mic input is forced to the Mac
...
- [X] make dashboard camera FPS much more fluid because its way too slow now; fix was to make it stream via TCP to computer
...
# the computer whilst Pepper still does the talking.
...
m.angleInterpolation(safe, [-0.3, -0.3, 1.0, 1.0, 0.0, 1.0], [3.0]*6, True)
for _ in range(5):
    m.angleInterpolation(["RWristYaw"], [0.5], [0.8], True)
    m.angleInterpolation(["RWristYaw"], [-0.5], [0.8], True)
time.sleep(0.3)
m.angleInterpolation(safe, [1.0, -0.15, 1.2, 0.4, 0.0, 0.0], [3.0]*6, True)
m.setStiffnesses("RArm", 0.0)
