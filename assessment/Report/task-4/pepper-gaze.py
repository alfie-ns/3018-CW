...
Pepper-only build. The dual-mode (Mac mic / TTS / webcam) variant lives in
local-gaze.py for laptop testing. GAZE_LOCAL_CAMERA=true falls back to the
Mac webcam if Pepper's camera black-screens, with the rest still on Pepper.
...
Things noticed during testing:
    ...
    - Pepper camera streaming runs at about 10 fps.
...
import os, re, json, time, types, wave, struct, socket, textwrap, tempfile, threading, unicodedata
...
BASE_SYSTEM_PROMPT = """
You are GAZE: a *social-companionistic* robot running on a NAO humanoid robot. 
You are a therapeutic companion first, and an engaging game host second.

CONVERSATION GUIDELINES:
- NEVER say the words 'Pepper' or 'Gaze' in your replies. These are the user's wake-word; the robot's own TTS output must NEVER contain them or the microphone will pick up the tail of your speech and the wake-word gate will false-positive on the next turn. Refer to yourself ONLY in the first person ('I', 'me', 'your companion').
...
def say(ssh_tts, text):
    "Speak text via Pepper TTS."
    nao_say(ssh_tts, text)
    time.sleep(0.5) # let TTS drain so it doesn't hear itself

def record(ssh, energy_threshold,
           silence_secs: float = 1.2,
           record_max_secs: float = RECORD_MAX_SECS):
    "Record via Pepper mics; per-turn think-budget from AdaptiveEngine.recommend_think_budget()."
...
m.angleInterpolation(names, [0.2, -0.2, 0.5, 0.8], [2.0]*4, True)
time.sleep(1.0)
m.angleInterpolation(names, [1.4, -0.2, 0.5, 0.0], [2.0]*4, True)
...
def generate_game_question_internal(game_type_str: str, difficulty_str: str,
    ...
    """Dedicated sub-call for game question generation via OpenAI.
       recent: last ~30 question strings (verbatim) shown to the user.
       recent_answers: last ~30 correct answers; catches mode-collapse when GPT rephrases the question but keeps the same target.
       recent_game_types: last ~30 game-type choices; feeds a rotation hint that pushes GPT off a single-category attractor."""
    try:
    ...
    # Python-side variety seed; GPT-4.1 at t=1.0 still mode-collapses on this constrained task, so we inject a random token to break the attractor
    import secrets
    variety_seed = secrets.token_hex(3)

    prompt = f"""Generate {GAME_DESCRIPTIONS[gt]} at {DIFFICULTY_DESCRIPTIONS[diff]} difficulty.
        Variety seed (use this to randomise your number/letter choice; different each call): {variety_seed}
    ...
    if recent:
        bullets = "\n".join(f"- {q}" for q in recent[-30:])
        prompt += f"NOT re-use or closely mimic any of these recently-asked questions. Pick a clearly different number set / target / letter set so the user experiences fresh material every round: {bullets}"

    if recent_answers:
        ans_bullets = ", ".join(recent_answers[-30:])
        prompt += f"Also, the CORRECT ANSWER to your new question MUST NOT be any of these recent answers: {ans_bullets}. Even if you rephrase the question, do NOT land on the same target."

    if recent_game_types:
        # rotation hint: if the last 3 questions were all the same game type, push GPT off that attractor
        tail = recent_game_types[-3:]
        if len(tail) >= 3 and len(set(tail)) == 1:
            dominant = tail[0]
            prompt += f"The last three rounds were all '{dominant}' questions. Pick a DIFFERENT game type this round to keep the session varied."
...
def build_system_prompt(user_name: str = "") -> str:
    ...
    if user_name and user_name != "friend":
        return BASE_SYSTEM_PROMPT + f"The user's name is {user_name}. Address them by name when it feels natural (not in every sentence)."
...
def ask_for_name(ssh, ssh_tts, dashboard,
    ...
    whisper_name_prompt = """
    The user is giving their name, for example: Alfie, Ella, Salman, Alex, Tom, Priya, Arjun, Oladeanio, Tom Smith, etc.
    Transcribe the name exactly; do not substitute numbers or dates.
    """
    ...
    for attempt_text in attempts:
        ...
        # longer budget than a normal turn; first-time speakers pause before saying their name. Also relax the energy gate: names are short and often quieter than task answers; the one-off override avoids masking the name whilst the rest of the conversation keeps the tighter threshold.
        name_threshold = max(150, energy_threshold - 100)
        record(ssh, name_threshold,
               silence_secs=2.5, record_max_secs=10.0)
        ...
        try:
            ...
            ).choices[0].message.content.strip().strip(" .!?,'\"") # clip any puncuation that might've got through
        except Exception as e:
            print(f"  Name extraction failed ({e})"); continue
        if (not n or n.upper() == "FRIEND" or len(n.split()) > 2 or len(n) > 30
                or NAME_REJECT_YEAR.match(n) or n.lower() in NAME_REJECT_WORDS):
            print(f"  Rejected name candidate: {n!r}"); continue
        return n.title()
...
def conversation_loop(dashboard, face_model, face_cascade, speech_model,
    ...
    try:
        while True:
            ...
            _, silence_secs, record_max_secs = engine.recommend_think_budget(
            ...
            if vol_rms >= NAO_MIN_RMS_TO_TRANSCRIBE:
            ...
            if gesture_type in ("celebrate", "encourage"):
