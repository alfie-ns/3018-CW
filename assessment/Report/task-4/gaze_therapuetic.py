...
#  emotional state directly — shifting Pepper from game host to cognitive
#  companion, analogous to assistive robots in stroke rehabilitation.
#
#  Cognitive mapping:
#    Perceive  → sustained negative emotional/performance signals detected
#    Attend    → consecutive_negative counter crosses threshold
#    Anticipate → predict that continuing the game will worsen the user's state
#    Plan      → select appropriate intervention level (graduated escalation)
#    Execute   → deliver intervention (physical, conversational, or affirmative)
#    Learn     → log intervention outcome for the episodic memory layer
# ══════════════════════════════════════════════════════════════════════════════
...
def therapy_reframing(ssh_tts, engine: 'AdaptiveEngine',
                      conversation: list, personality: Personality) -> str:
    """
    ...
    natural, personality-appropriate response rather than a canned script.
    ...
    reframe_prompt = (
        f"{PERSONALITY_PROMPTS[personality]}\n\n"
...
def execute_therapy(therapy_type: TherapyType, ssh, ssh_tts,
                    engine: 'AdaptiveEngine', conversation: list,
    ...
    if therapy_type == TherapyType.MINDFULNESS:
        ...
        dialogue = therapy_reframing(ssh_tts, engine, conversation, personality)
        engine.mark_therapy_given()
        return dialogue
        ...
        while engine.round_number <= MAX_ROUNDS:
            ...
            if decision.therapy_break != TherapyType.NONE:
                therapy_dialogue = execute_therapy(
                    ...
                    engine, conversation, personality
                )
                if therapy_dialogue:
                    ...
                    if decision.therapy_break in (TherapyType.MINDFULNESS,
                        ...
                        if check_audio_volume():
                            ...
                            feedback = parse_feedback(therapy_response)
                            if "game" in feedback:
                                engine.current_game = feedback["game"]
                                engine.game_switch_count += 1
                            if "personality" in feedback:
                                personality = feedback["personality"]
                                conversation[0]["content"] = (
                                    "You are a Pepper robot game host called GAZE. "
                                    "You play interactive games with users and adapt "
                                    "based on their emotional state and performance. "
