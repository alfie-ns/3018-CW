...
It doesn't just react but *anticipates* (plans accordingly to its belief about the user).
...
Code-specific: a Python simulation that shows all of this happening visually; the belief updating, the action choices, the outcomes over continous interaction, {continue post code drafting of other things}

Why it's novel: wrapping an LLM (which is good at reading people but has no memory; no planning ability, etc) inside a POMDP (which provides the memory and planning the LLM lacks).
